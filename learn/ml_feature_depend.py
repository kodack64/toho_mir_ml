...
def featureDependency(useFeature,target,topn=20):
    cv = 4
    # load data and split
...
def featureDependencyWithRFE(useFeature,target,topn=20):
    cv = 4
    # load data and split
    fin = open("./learn/data/"+useFeature+"_thbgm.pkl","rb")
    X_th = pickle.load(fin)
    ...
    rfeSelect = topn
    rfe = RFE(LinearSVC(),rfeSelect,step=1)
    rfe.fit(X,Y)
    featNames = ml_feature_name.getFeatureName(useFeature)
    featNames = np.array(featNames)[rfe.support_]
    print(featNames)
    X = np.array(X)[:,rfe.support_]
    Y = np.array(Y)
    featureCount = rfeSelect
    print(X.shape)
    print(Y.shape)
    ...
    if(test_size > 0.0):
        Y_p = clf.predict(X_)
        print(classification_report(Y_,Y_p))
